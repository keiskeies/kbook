...
public class EmailNotificationService {
    ...
    @Value("${spring.mail.send-enabled:false}")
    private boolean mailSendEnabled;

    @Value("${notification.base-url:http://localhost:5173}")
    private String baseUrl;

    @Value("${notification.invitation.expire-hours:72}")
    private int invitationExpireHours;

    @Value("${notification.comment-reply-thresholds:10,50,100}")
    private String commentReplyThresholdsStr;

    @Value("${notification.comment-like-thresholds:10,50,100,500}")
    private String commentLikeThresholdsStr;
    ...
    public String sendInvitation(String toEmail, String inviterName, String bookTitle) {
        ...
        redisTemplate.opsForValue().set(redisKey, inviteData, Duration.ofHours(invitationExpireHours));
        ...
        Map<String, Object> data = Map.of(
            ...
            "expireHours", invitationExpireHours,
            "actionText", "接受邀请"
        );
    ...
    public void checkAndSendReplyThresholdNotification(
        ...
        Set<Integer> thresholds = parseThresholds(commentReplyThresholdsStr);
    ...
    public void checkAndSendLikeThresholdNotification(
        ...
        Set<Integer> thresholds = parseThresholds(commentLikeThresholdsStr);
        for (int threshold : thresholds) {
...
""".formatted(getEmailBody(type, data), now, baseUrl);
    }
