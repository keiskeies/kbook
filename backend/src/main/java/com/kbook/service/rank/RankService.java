 ...
 * <p>
 * 负责管理平台排行榜数据，当前仅维护综合热度榜（hot），
 * 按阅读量、AI问答、圆桌讨论、辩论、书架收藏等多维度评分降序排列。
 * <p>
 * 缓存策略：
 * - 榜单缓存图书 ID 列表（逗号分隔），取用时通过 BookRepository 查询最新数据
 * - 启动时预填充（@PostConstruct），之后每2小时定时刷新（@Scheduled）
 * - 缓存未命中时降级到数据库查询
 */
@Slf4j
@Service
...
public class RankService {
    ...
    /** Redis Key：热度榜缓存，存储综合热度降序的前300本书籍ID */
    private static final String HOT_RANK_KEY = "kbook:rank:hot";
    ...
    /**
     * 启动时预填充热度榜缓存，避免首次请求等待定时刷新
     */
    @PostConstruct
    public void initHotRankCache() {
        log.info("启动时预填充热度榜缓存...");
        refreshHotRankCache();
    }

    /**
     * 刷新热度榜缓存 — 委托 RecommendService 执行综合评分 SQL 并缓存前 300 本
     */
    private void refreshHotRankCache() {
        try {
            List<BookProjection> hotBooks = recommendService.getHotBooks();
            cacheBookIds(HOT_RANK_KEY, hotBooks);
            log.debug("热度榜缓存刷新完成: {}本", hotBooks.size());
        } catch (Exception e) {
            log.warn("刷新热度榜缓存失败: {}", e.getMessage());
        }
    }
    ...
    /**
     * 从缓存读取指定页码的热度榜（综合阅读量、AI问答、圆桌、辩论、书架收藏等维度评分）
     */
    @LogAction("获取热度榜")
    public PageResult<BookProjection> getHotRank(int page, int size) {
        return getCachedPage(HOT_RANK_KEY, page, size);
    }
    ...
    private PageResult<BookProjection> loadFromDb(String cacheKey, int page, int size) {
        if (HOT_RANK_KEY.equals(cacheKey)) {
            List<BookProjection> hotBooks = recommendService.getHotBooks();
            int total = hotBooks.size();
            int fromIndex = (page - 1) * size;
            if (fromIndex >= total) return PageResult.of(List.of(), total, page, size);
            int toIndex = Math.min(fromIndex + size, total);
            return PageResult.of(hotBooks.subList(fromIndex, toIndex), total, page, size);
        }
